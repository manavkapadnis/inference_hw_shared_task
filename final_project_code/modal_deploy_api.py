...
# Create Modal app
app = modal.App("mkapadni-inference-system")
...
def extract_graph_params_from_prompt(prompt: str) -> Optional[Tuple[List[List[int]], int, int]]:
    """
    Extract graph parameters (edges, N, P) from user prompt using robust regex.
    Returns (edges, N, P) if successful, None otherwise.
    Adapted from dataset_handlers.py parse_response logic
    """
    edges = None
    N = None
    P = None
    ...
    # Try extracting N
    n_patterns = [
        r'graph with (\\d+) nodes',
        r'(\\d+) nodes? \\(numbered',
        r'nodes? numbered 0 to (\\d+)',
        r'N\\s*=\\s*(\\d+)',
    ]
    for pattern in n_patterns:
        ...
        if match:
            N = int(match.group(1))
            # If "numbered 0 to X", then N = X+1
            if "numbered 0 to" in match.group(0).lower():
                N = N + 1
            break
    
    # Try extracting P
    p_patterns = [
        r'top (\\d+) shortest path',
        r'find (\\d+) shortest',
        r'P\\s*=\\s*(\\d+)',
    ]
    ...
    # Try extracting edges from prompt
    edges_from_prompt = []
    edge_patterns = [
        # Format: "0 -> 1, weight: 5"
        r'(\\d+)\\s*->\\s*(\\d+),?\\s*weight:?\\s*(\\d+)',
        # Format: "edge (0, 1) with weight 5"
        r'edge\\s*\\(?(\\d+),?\\s*(\\d+)\\)?\\s*(?:with)?\\s*weight:?\\s*(\\d+)',
        # Format: "[0, 1, 5]"
        r'\\[(\\d+),\\s*(\\d+),\\s*(\\d+)\\]',
    ]
    ...
    for pattern in edge_patterns:
        ...
        for match in matches:
            try:
                ...
                edges_from_prompt.append([s, d, w])
            except:
                continue
    
    if edges_from_prompt:
        # Deduplicate
        edges = [list(x) for x in set(tuple(e) for e in edges_from_prompt)]
    
    # Also try extracting edges in array format: edges=[[...]]
    edges_pattern = r'edges\\s*=\\s*(\\[\\s*\\[[\\d,\\s\\[\\]]+\\]\\s*\\])'
    edges_match = re.search(edges_pattern, prompt, re.DOTALL)
    if edges_match and not edges:
        try:
    ...
    if edges and N is not None and P is not None and N > 0 and P > 0:
        # Normalize edges
        norm_edges = []
        for e in edges:
...
class InferenceAPI:
    ...
    def load_model(self):
        ...
        # Get configuration
        large_model = os.environ.get("LARGE_MODEL", "Qwen/Qwen3-8B")
        small_model = os.environ.get("SMALL_MODEL", "Qwen/Qwen3-4B")
        ...
        self.max_batch_sizes = {
            "graph": 4,      # Complex, needs accuracy
            "mmlu": 8,       # Medium complexity
            "infobench": 6   # Variable length
        }
    ...
    async def completions(self, request: dict):
        ...
        timeout = 590.0  # 590 seconds (10 sec buffer before Modal timeout)
        future = asyncio.Future()
