...
class Block(nn.Module):
    ...
    def forward(self, x, prev_layer_kv=None):
        ...
        attn_output, cur_layer_kv = self.attn(self.ln_1(x), prev_layer_kv)
        x = x + attn_output  # LEAVE THIS THE SAME
        x = x + self.mlp(self.ln_2(x))
        return x, cur_layer_kv # TODO: Uncomment this part once the rest of the function is implemented
...
class GPT(nn.Module):
    ...
    def forward(self, idx, past_kv_caches=None): # past_kv_cache is a list of caches, one per layer
        ...
        # TODO: get sequence length from cache if it exists
        if past_kv_caches is not None:
            t_past = past_kv_caches[0]["k"].size(-2)  # Get seq length from first layer's cache
        ...
        assert t + t_past <= self.config.block_size, f"Cannot forward sequence of length {t}, block size is only {self.config.block_size}"
        ...
        if past_kv_caches is None:
            past_kv_caches = [None] * self.config.n_layer

        new_kv_caches = []
        for i, block in enumerate(self.transformer.h):
            x, new_layer_kv = block(x, past_kv_caches[i]) # TODO: adjust this line to use your modified forward() function (and kv)
            new_kv_caches.append(new_layer_kv) # TODO: append to new_kv_caches
