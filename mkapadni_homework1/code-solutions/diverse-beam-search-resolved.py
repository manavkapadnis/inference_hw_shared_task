...
def diverse_beam_search(model, tokenizer, prompt, beam_width=6, num_groups=3, max_length=50, device='cpu', diversity_penalty=1.0):
    ...
    beams = [[(input_ids, 0.0)] for _ in range(num_groups)] # BUG 1: initialize log-score (0.0) instead of prob (1.0)
        ...
        for g in range(num_groups):
            ...
            for seq, score in beams[g]:
                with torch.no_grad():
                    ...
                    log_probs = torch.log_softmax(logits, dim=-1)  # BUG 1: use log-probabilities for additive scoring
                # Penalize tokens used by previous groups
                penalized_probs = log_probs.clone()  # BUG 3: apply penalty in log-space to the distribution used for selection
                for token in prev_tokens_counts:
                    penalized_probs[0, token] -= (diversity_penalty * prev_tokens_counts[token])  # BUG 3: subtract Hamming penalty in log-space
                topk_probs, topk_ids = torch.topk(penalized_probs, group_size)  # BUG 3: select top-k from penalized distribution
                for i in range(group_size):
                    next_id = topk_ids[0, i].unsqueeze(0)
                    next_score = score + topk_probs[0, i].item()  # BUG 1: accumulate in log-space (add), not multiply probs
            ...
            beams[g] = sorted(new_beams, key=lambda x: x[1], reverse=True)[:group_size]  # BUG 2: prune each group to group_size (no per-step length norm)
