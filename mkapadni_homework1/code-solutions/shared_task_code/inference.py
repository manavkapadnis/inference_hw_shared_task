...
def extract_infobench_answer(text: str) -> str:
    """
    Extracts the assistant's response after the <think>...</think> block.
    """
    cleaned = re.sub(r".*?</think>\s*", "", text, flags=re.DOTALL)
    return cleaned.strip()

def load_hf_model_and_tokenizer(model_name: str, device: str = None):
    device = device or ("cuda" if torch.cuda.is_available() else "cpu")
    tokenizer = AutoTokenizer.from_pretrained(model_name)
    model = AutoModelForCausalLM.from_pretrained(model_name, torch_dtype="auto", device_map="auto" if device == "cuda" else None)
    model.to(device)
    # Ensure pad token to avoid warnings/crashes
...
def is_chat_like(tokenizer) -> bool:
    # Qwen chat/instruct models provide a chat template; use if available
    return hasattr(tokenizer, "apply_chat_template") and tokenizer.chat_template is not None

def build_inputs(tokenizer, prompt: str, device: str, chat_mode: bool, thinking: bool):
    if chat_mode:
        messages = [
...
def get_decoding_kwargs(setting: str):
    # Map of requested decoding settings:
    # - default: use model.generation_config (only override max_new_tokens)
    # - greedy: greedy search
    # - temp_0_25, temp_1_5: temperature sampling
    # - beam_3, beam_25: beam search
    # - typical: locally typical sampling (typical_p)
    if setting == "default":
        return {"use_default": True}
    if setting == "greedy":
        return {"do_sample": False}
    if setting == "temp_0_25":
        return {"do_sample": True, "temperature": 0.25, "top_p": 1.0}
    if setting == "temp_1_5":
        return {"do_sample": True, "temperature": 1.5, "top_p": 1.0}
    if setting == "beam_3":
        return {"do_sample": False, "num_beams": 3, "early_stopping": True}
    if setting == "beam_25":
        return {"do_sample": False, "num_beams": 25, "early_stopping": True}
    if setting == "typical":
        # Locally typical sampling via typical_p
        return {"do_sample": True, "typical_p": 0.9, "top_p": 1.0}
    raise ValueError(f"Unknown decoding setting: {setting}")

@torch.inference_mode()
def hf_generate_once(model, tokenizer, prompt: str, setting: str, max_new_tokens: int = 128):
    device = next(model.parameters()).device
    chat_mode = is_chat_like(tokenizer) 
    if model == "Qwen/Qwen3-4B-Instruct-2507":
        inputs = build_inputs(tokenizer, prompt, device, chat_mode, True) # this is to tell that it is non thinking model, don't give that param
    else:
        inputs = build_inputs(tokenizer, prompt, device, chat_mode, False)

    kwargs = get_decoding_kwargs(setting)
    if kwargs.pop("use_default", False):
...
def generate_problem_prompt(dataset_name: str, example: str) -> str:
    if dataset_name == "MMLU":
        ...
        prompt = f"The following is a multiple choice question (with answers) about {format_subject(example['subject'])}. Strictly give output the answer in the format of \"The answer is (X)\" at the end.\n\n"
...
def convert_llm_response_to_solution(llm_response: str, dataset_name: str) -> str:
    if dataset_name == "MMLU":
        ...
        return extract_infobench_answer(llm_response)
    else:
        raise ValueError(f"Dataset {dataset_name} not found")
...
def run_evaluation(examples: List[Dict[str, Any]], model: str, api_key: str, task: str, openai_api_key: str="api_key_value") -> Dict[str, Any]:
    """
